/*
	Forked from discontinued module : https://www.npmjs.com/package/crypto-random-string
	Modified by Fatih Aziz
	License: MIT
*/
...
const generateForCustomCharacters = (length, characters) => {
  ...
  while (stringLength < length) { // In case we had many bad values, which may happen for character sets of size above 0x8000 but close to it
    ...
    while (entropyPosition < entropyLength && stringLength < length) {
      ...
      string += characters[entropyValue % characterCount] // eslint-disable-line @typescript-eslint/restrict-plus-operands
...
const createGenerator = (generateForCustomCharacters, generateRandomBytes) => ({ length, type, characters }: createGeneratorRequest) => {
  if (!length) 
