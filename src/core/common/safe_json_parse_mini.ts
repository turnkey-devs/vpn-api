...
function ensureProperties(object) {
  ...
  function visit(object_) {
    ...
    if (object_ instanceof Error) {
      try {
        const errorResult = visit(String(`[${ object_.name }]: ${ object_.message } -> ${ object_.stack }`))
        seen.pop()
        return errorResult
      } catch (error) {
        return throwsMessage(error)
...
export const safeJsonParse = <T, R>(string_: T): (T extends { [K in keyof T]?: T[K] } ? T : R) | undefined => {
  try {
